...
use crate::{
    ...
    models::enrollments::{CreateEnrollmentsDto, Enrollment, UpdateEnrollmentsDto},
};
...
pub async fn get_enrollments_by_course(
    ...
    let submissions: Result<Vec<Enrollment>, sqlx::Error> =
        sqlx::query_as::<_, Enrollment>("SELECT * FROM get_enrollments_by_course($1)")
            .bind(id)
...
pub async fn get_enrollments_by_user(
    ...
    let submissions: Result<Vec<Enrollment>, sqlx::Error> =
        sqlx::query_as::<_, Enrollment>("SELECT * FROM get_enrollments_by_user($1)")
            .bind(id)
            .fetch_all(pool.get_ref())
            .await;
...
pub async fn create_enrollments(
    ...
    let result = sqlx::query("SELECT create_enrollment($1, $2, $3, $4, $5, $6)")
...
pub async fn delete_enrollment(pool: web::Data<PgPool>, path: web::Path<i32>) -> impl Responder {
    let id = path.into_inner();
...
pub async fn update_enrollment(
    ...
    let result: Result<Enrollment, sqlx::Error> =
        sqlx::query_as::<_, Enrollment>("SELECT * FROM update_enrollment($1, $2, $3, $4, $5)")
            .bind(id)
